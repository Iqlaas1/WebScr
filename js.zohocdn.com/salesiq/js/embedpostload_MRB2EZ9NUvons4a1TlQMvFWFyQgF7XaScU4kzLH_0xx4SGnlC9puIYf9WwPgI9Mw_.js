jQuery(function(c) {
    var r, p, h = {},
        d = void 0,
        l = void 0,
        f = 800,
        w = void 0,
        m = void 0,
        v = void 0,
        g = void 0,
        z = void 0,
        x = void 0,
        W = void 0,
        u = void 0,
        s = void 0,
        T = void 0,
        b = {},
        y = {
            northwest: "top",
            north: "top",
            northeast: "top",
            west: "left",
            east: "right",
            southwest: "bottom",
            south: "bottom",
            southeast: "bottom"
        },
        E = {
            callout: "north",
            autohide: !0,
            mousetrack: !1,
            mouseEnter: !1,
            showShortcutKey: !0,
            contentType: "text",
            positionAlterable: !0,
            calloutHideMode: !1
        };

    function n(t) {
        this._windowEle = c(window), this._windowWidth = this._windowEle.width();
        var a = this;
        this._windowEle.on("resize." + this.widgetName, function(t) {
            a._windowWidth = a._windowEle.width()
        }), this._documentEle = c(document), this._documentEle.on("keydown." + this.widgetName, function(t) {
            27 == t.keyCode && O(t)
        }), c(t).on("buttondisabled", function(t, o) {
            var e = c(o.element),
                i = e.attr("title");
            o.disabled ? (e.removeAttr("title"), e.data("ztooltip", i)) : e.attr("title", e.data("ztooltip"))
        }), c(t).on("mouseover mouseup", function(t) {
            var o = function(t, o) {
                    var e = "[" + o + "]";
                    try {
                        t = t.is(e) ? t : c(t.parents(e).get(0))
                    } catch (t) {}
                    return t
                }(c(t.target), "title"),
                e = o.attr("title"),
                i = o.attr("contentType");
            if (e && o.length && "document" !== o[0].nodeName.toLowerCase()) {
                if (l && clearTimeout(l), t.stopPropagation(), t.preventDefault(), o.attr("disabled") || o.hasClass("ui-state-disabled") || o.hasClass("ui-ztooltip-ignore")) return _(o), O(), !1;
                _(o), h = c.extend({}, E, function(t, o) {
                    try {
                        var e = c.parseJSON(o);
                        return e.componentOptions ? e.componentOptions : e = {
                            content: o
                        }
                    } catch (t) {
                        return eleOptions = {
                            content: o
                        }, eleOptions
                    }
                }(0, e)), i && (h.contentType = i);
                var n = o.attr("callout");
                void 0 !== n && "" !== y[n] && (h.callout = n), c(w).length && c(w).find(".ui-ztooltip-close").remove(), f = 800,
                    function(o, e) {
                        var i = "html" == h.contentType;
                        !0 !== h.mouseEnter && !0 !== h.mousetrack || (h.callout = "northwest");
                        var n = function(t) {
                            var o = !!w,
                                e = "ui-corner-all",
                                i = h.ajaxContent ? "ui-ztooltip-ajaxcontent " : h.autohide ? "ui-ztooltip-content " : "ui-ztooltip-contentwithclose ";
                            if (o) f = 0;
                            else {
                                w = c("<div>").attr("class", "ui-ztooltip ui-widget"), x = void 0;
                                var n = c("<div>").attr("class", "ui-ztooltip-overlaydiv ui-corner-all");
                                g = c("<div>").attr("class", "ui-ztooltip-header ui-corner-top"), z = c("<div>").attr({
                                    class: i + e,
                                    id: "ui-ztooltip-content"
                                }), W = c("<span>").attr("class", "ui-ztooltip-img " + h.iconclass).appendTo(n), g.appendTo(n), z.appendTo(n), n.appendTo(w), c("body").append(w)
                            }
                            g.css("display", ""), W.css("display", ""), h.iconclass && W.css({
                                display: "inline-block"
                            });
                            h.title && (g.text(h.title), e = "ui-corner-bottom", g.css({
                                display: "inline-block"
                            }));
                            if (!h.autohide) {
                                var a = c("<span>").attr("class", "ui-ztooltip-close ui-icon ui-components-icon").on("click", function(t) {
                                    c(m).off("mousemove"), O(t), c(m).attr("title", v)
                                });
                                a.appendTo(w), c(m).attr("noAutoHide", !0)
                            }
                            if (z.attr("class", i + " " + e), !h.ajaxContent) {
                                p = h.content;
                                var l = "html" == h.contentType;
                                if (l && (p = getEncodedText(p)), p = Smiley.replace(p), l && h.isContentElementId) {
                                    var u = c("<div>").append(c("#" + h.content).show());
                                    p = u.html()
                                }
                                if (b && c.ui && c.ui.shortcutkeys && b.showShortcutKey) {
                                    t = t.is("label.ui-button") ? t.prev("input") : t;
                                    var s = c(document).shortcutkeys("getShortcutKey", t);
                                    s && (p = p + " (" + s + ")")
                                }
                            }
                            d && d.remove();
                            d = w.clone(), c("body").append(d);
                            var l = "html" == h.contentType;
                            d.removeAttr("style").hide(), d.find("#ui-ztooltip-content").removeAttr("style"), d.find("#ui-ztooltip-content")[l ? "html" : "text"](p), h.callout && !h.ajaxContent ? (x || (x = c("<div>").attr("class", "ui-ztooltip-callout").appendTo(w)), r = k(h.callout, x)) : x = void 0;
                            return w
                        }(e);
                        if (n) {
                            var a = void 0;
                            o.originalEvent ? a = h.callout && !h.ajaxContent ? F(o) : {
                                left: o.pageX + 10,
                                top: o.pageY + 10
                            } : ((a = e.offset()).left += e.width(), a.top += e.height()), a = H(a, e, d), clearTimeout(u), clearTimeout(s), u = setTimeout(function() {
                                if (e.is(":visible") || e.is("option")) {
                                    if (h.ajaxContent) {
                                        var t = c("<div>").attr("class", "ui-ztooltip-content-loading");
                                        c(w).find(".ui-ztooltip-ajaxcontent").append(t), s = setTimeout(function() {
                                            c.ajax({
                                                type: "GET",
                                                url: h.ajaxContent,
                                                data: "",
                                                success: function(t) {
                                                    c(w).find(".ui-ztooltip-ajaxcontent").html(t)
                                                },
                                                error: function() {
                                                    c(w).find(".ui-ztooltip-ajaxcontent").empty()
                                                }
                                            })
                                        })
                                    }
                                    h.timeout && h.autohide && (T = setTimeout(function() {
                                        O(o)
                                    }, parseInt(h.timeout))), n.show(1, function() {
                                        z[i ? "html" : "text"](p), x.css(r), (a = S(o, n, a, e)) && n.css(a), h.calloutHideMode && w.find(".ui-ztooltip-callout").hide()
                                    })
                                }
                            }, f)
                        }
                    }(t, o), v = e, (m = o).on("click.ztooltip", function(t) {
                        O()
                    }), o.off("mouseout.ztooltip").on("mouseout.ztooltip", function(t) {
                        h.timeout && clearTimeout(T), l && clearTimeout(l), l = setTimeout(function() {
                            h.autohide && (o.unbind("mousemove"), O(t))
                        }, 200), _(o, !0)
                    }), a._documentEle.off("mousedown.ztooltip").on("mousedown.ztooltip", function(t) {
                        h.autohide && (o.unbind("mousemove"), O(t)), _(o, !0)
                    }), h.mousetrack && o.off("mousemove.ztooltip").on("mousemove.ztooltip", function(t) {
                        ! function(t, o) {
                            var e = c(w),
                                i = (e.width(), e.height(), h.callout && !h.ajaxContent ? F(t) : {
                                    left: t.pageX + 5,
                                    top: t.pageY + 5
                                });
                            i = S(t, w, i, o), e.width() + parseInt(i.left) > this._windowWidth && e.width(this._windowWidth - (parseInt(i.left) + 50));
                            e.css(i)
                        }(t, o)
                    })
            }
        })
    }

    function _(t, n) {
        var o = n ? t.data("parents") : t.parents("[title]");
        n ? t.data("parents", void 0) : (o.push(t), t.data("parents", o)), o && c.each(o, function(t, o) {
            var e = "",
                i = (o = c(o)).attr("title");
            n ? e = i != o.data("title") && "" != i ? i : o.data("title") : o.data("title", i), o.attr("title", e)
        })
    }

    function H(t, o, e, i) {
        var n = h.callout ? h.callout : i,
            a = o.offset(),
            l = a.left,
            u = a.top;
        return o.is("option") && (l = o.parent("select").offset().left, u = o.parent("select").offset().top + o.offset.top), n && (-1 != n.indexOf("north") ? (h.mouseEnter || h.mousetrack || (t.left = "northwest" === n ? l : "northeast" === n ? l - (e.width() - o.outerWidth()) : l - 8 - (e.width() / 2 - 8) + o.outerWidth() / 2), t.top = u + o.outerHeight() + 9) : -1 != n.indexOf("south") ? (t.left = "southwest" === n ? l : "southeast" === n ? l - (e.width() - o.outerWidth()) : l - 8 - (e.width() / 2 - 8) + o.outerWidth() / 2, t.top = u - e.outerHeight() - 9) : "west" === n ? (t.left = parseInt(l + o.outerWidth() + 9), t.top = u - e.height() / 2 + o.outerHeight() / 2) : "east" === n && (t.left = parseInt(l - e.width() - 9), t.top = u - e.height() / 2 + o.outerHeight() / 2)), t
    }

    function k(t, o) {
        if (!w) return !1;
        var e = {};
        o.removeAttr("class").attr("class", "ui-ztooltip-callout ui-ztooltip-callout-" + y[t]);
        var i = o.outerWidth(),
            n = o.outerHeight(),
            a = d.width(),
            l = d.height();
        switch (t) {
            case "north":
                e = {
                    left: a / 2 - i / 2 + "px",
                    top: "-" + (n - 1) + "px"
                };
                break;
            case "west":
                e = {
                    left: "-" + (i - 1) + "px",
                    top: l / 2 - n / 2 + "px"
                };
                break;
            case "east":
                e = {
                    left: a - 1 + "px",
                    top: l / 2 - n / 2 + "px"
                };
                break;
            case "south":
                e = {
                    left: a / 2 - i / 2 + "px",
                    top: l - 1 + "px"
                };
                break;
            case "northeast":
                e = {
                    left: a - 2 * i + "px",
                    top: "-" + (n - 1) + "px"
                };
                break;
            case "northwest":
                e = {
                    left: i + "px",
                    top: "-" + (n - 1) + "px"
                };
                break;
            case "southwest":
                e = {
                    left: i + "px",
                    top: l - 1 + "px"
                };
                break;
            case "southeast":
                e = {
                    left: a - 2 * i + "px",
                    top: l - 1 + "px"
                };
                break;
            default:
                o.addClass("ui-ztooltip-callout-top"), e = {
                    left: i + "px",
                    top: "-" + (n - 1) + "px"
                }
        }
        return e
    }
    var i = {
        north: function(t) {
            return {
                left: t.pageX - t.tooltipWidth / 2,
                top: t.pageY + (t.calloutHeight + t._OFFSET)
            }
        },
        west: function(t) {
            return {
                left: t.pageX + (t.calloutWidth + t._OFFSET),
                top: t.pageY - t.tooltipHeight / 2
            }
        },
        east: function(t) {
            return {
                left: t.pageX - (t.calloutWidth + t._OFFSET) - t.tooltipWidth,
                top: t.pageY - t.tooltipHeight / 2
            }
        },
        south: function(t) {
            return {
                left: t.pageX - t.tooltipWidth / 2,
                top: t.pageY - (t.calloutHeight + t._OFFSET) - t.tooltipHeight
            }
        },
        northeast: function(t) {
            return {
                left: t.pageX - (t.tooltipWidth - (t.calloutWidth + t.calloutWidth / 2)),
                top: t.pageY + t.calloutHeight + t._OFFSET
            }
        },
        northwest: function(t) {
            return {
                left: t.pageX - (t.calloutWidth + t.calloutWidth / 2),
                top: t.pageY + t.calloutHeight + t._OFFSET
            }
        },
        southwest: function(t) {
            return {
                left: t.pageX - (t.calloutWidth + t.calloutWidth / 2),
                top: t.pageY - (t.calloutHeight + t._OFFSET) - t.tooltipHeight
            }
        },
        southeast: function(t) {
            return {
                left: t.pageX - (t.tooltipWidth - (t.calloutWidth + t.calloutWidth / 2)),
                top: t.pageY - (t.calloutHeight + t._OFFSET) - t.tooltipHeight
            }
        },
        defaultFn: function(t) {
            return i.north(t, t.calloutHeight, t.calloutWidth)
        }
    };

    function F(t) {
        t.tooltipWidth = w.outerWidth(), t.tooltipHeight = w.height();
        if (t._OFFSET = 10, !w) return !1;
        var o = w.find(".ui-ztooltip-callout");
        t.calloutWidth = o.outerWidth(), t.calloutHeight = o.outerHeight();
        var e = i[h.callout];
        return e || (e = i.defaultFn), e(t)
    }

    function O(t) {
        w && ("html" === h.contentType && h.isContentElementId && c(w).find("#" + h.content).appendTo("body").hide(), c(w).remove(), w = void 0)
    }

    function S(t, o, e, i) {
        if (o && e) {
            var n, a = e.top - this._documentEle.scrollTop(),
                l = e.left - this._documentEle.scrollLeft(),
                u = l + o.width(),
                s = a + o.height(),
                r = void 0;
            if (u >= this._windowWidth ? (e.left = this._windowWidth - (o.outerWidth(!0) + 5), e.left + o.width() - i.offset().left <= 15 && (h.callout = "east"), r = h.callout) : l < 0 && (e.left = e.left + 5 - l, i.offset().left + i.width() - this._documentEle.scrollLeft() <= 20 && (h.callout = "west"), r = h.callout), s >= this._windowEle.height() ? (e.top = i.offset().top - o.outerHeight() - 9, r = "south", n = a) : a < 0 && (e.top = i.offset().top + i.outerHeight() + 9, r = "north", n = a), r) ! function(t, o, e, i, n, a, l) {
                var u = h.callout ? h.callout : o;
                switch (o) {
                    case "north":
                        u = "south" == u ? "north" : "southeast" == u ? "northeast" : "southwest" == u ? "northwest" : u;
                        break;
                    case "south":
                        u = "north" == u ? "south" : "northeast" == u ? "southeast" : "northwest" == u ? "southwest" : u;
                        break;
                    case "west":
                        u = "east" == u ? "west" : u;
                        break;
                    case "east":
                        u = "west" == u ? "east" : u
                }(function(t, o, e, i, n, a, l, u) {
                    if (e) {
                        h.callout = o;
                        var s = e.position();
                        u && ((s = k(o, e)) || (s = e.position())), "east" === o || "west" === o ? (s = k(o, e), a = H(a, c(t.target), i, n)) : "northwest" === o || "northeast" === o || "southwest" === o || "southeast" === o ? s = k(o, e) : l < 0 ? (s.left = i.width() / 2 - e.outerWidth() / 2 + l - 5, s.left <= 10 && (s.left = 10)) : (s.left = i.width() / 2 - e.outerWidth() / 2 + (l - a.left), i.width() - s.left <= 20 && (s.left = i.width() - 20)), s && e.css(s)
                    }
                })(t, u, e, i, o, n, a, l)
            }(t, r, o.find(".ui-ztooltip-callout"), o, e, l, n)
        }
        return e
    }
    c.fn.ztooltip = function(t, o, e) {
        var i = c(this);
        b = c.extend({}, E, "object" == typeof t && t), i.data("z-ztooltip") ? "remove" === t && O() : (n(i), i.data("z-ztooltip", !0))
    }, c.fn.ztooltip.remove = O, !$Support.isMobileWidth() && $Support.isIframeContentLoaded() && c(document).ztooltip()
});
LSDragDrop = {
    onDropEvent: function(e, t, i) {
        if (-1 != e) try {
            t.preventDefault();
            var a = i || t.dataTransfer.files;
            this.nooffiles = a.length, this.pos = 0, this.uploadFile(e, a)
        } catch (e) {}
    },
    uploadFile: function(i, a, e, t) {
        var o = this,
            s = (new Date).getTime(),
            n = null != e ? e : "chat_attachment",
            l = {
                progress: function(e, t) {
                    $UI.showUploadProgress({
                        fileobj: a[o.pos - 1],
                        prgval: e,
                        msgid: t,
                        att_type: n
                    }, i)
                },
                complete: function() {
                    r()
                },
                success: function() {
                    r()
                }
            };

        function r() {
            $UI.hideUploadProgress(s), $Support.Util.handleCallbacks("chat.file", {}), o.pos < o.nooffiles && (s = (new Date).getTime(), o.uploadFileUsingPost(i, a, o.pos, l, s), o.pos++)
        }
        o.uploadFileUsingPost(i, a, o.pos, l, s, n, t), o.pos++
    },
    getFormData: function(e, t) {
        var i = new FormData;
        return $.each(e, function(e, t) {
            i.append(e, t)
        }), i
    },
    uploadFileUsingPost: function(e, t, i, a, o, s, n) {
        try {
            var l = ($Support.EmbedObj || {}).isembedrestapienabled || !1,
                r = t[i],
                d = this,
                p = $Support.EmbedObj;
            if ("image/svg+xml" == r.type || window.ZSIQFileUpload && !ZSIQFileUpload.isWhitelistedFileType(r)) {
                var c = t && 1 == t.length ? "fileupload.invalid" : "filesupload.invalid.ignore";
                return ZSIQConversationManager.getChatWinObjById(e).showBanner(LSResource.getRealValue(c), !0), d.pos++, void a.success()
            }
            var g = null != s ? s : "chat_attachment";
            if (r.size <= 52428800) {
                var u, m = {};
                try {
                    u = l ? (m = {
                        file: r,
                        client_message_id: o,
                        timezone_offset: -1 * (new Date).getTimezoneOffset() * 6e4,
                        session_id: LSMessanger.getSid(),
                        uts_session_id: $Support.getParent().$UTSHandler.getUTSSID(),
                        att_type: g || "chat_attachment"
                    }, "voice_message" == g && (m.media_id = n), CommonUtil.getFormData(m)) : (m = {
                        chid: e,
                        dname: p.visitorname,
                        mod: "att",
                        sid: LSMessanger.getSid(),
                        lsid: p.livelsid,
                        sender: p.vwmsid,
                        msgid: o,
                        file1: r,
                        att_type: g
                    }, "voice_message" == g && (m.m_id = n), d.getFormData(m))
                } catch (e) {
                    throw e
                }
                try {
                    (l ? CommonUtil.v2handleFileUpload : CommonUtil.handleFileUpload)({
                        callbacks: a,
                        formdata: u,
                        type: r.type,
                        msgid: o,
                        isAudio: "voice_message" == g
                    })
                } catch (e) {}
                return
            }
            var f = r.name;
            if (f = null == f || null == f ? "file1" : f, d.pos = i + 1, null == t[d.pos] && null == t[d.pos]) return $UI.showBanner(LSResource.getRealValue("infomsg.filesharing.sizeexceed", f + ",50"), !0), void $UI.hideUploadProgress(o);
            if (!confirm(LSResource.getRealValue("infomsg.filesharing.sizeexceedproceednext", f + ",50"))) return;
            d.uploadFileUsingPost(e, t, d.pos, a, o)
        } catch (e) {}
    }
};
var PasteImage = {};
PasteImage = {
    isUser: !1,
    setIsUser: function(e) {
        this.isUser = e
    },
    onPaste: function(e, t) {
        if ($Support.isChatExist()) {
            if (this.txtarea = t || null, window.clipboardData && window.clipboardData.getData("Text")) return e.preventDefault(), this.insertText(n);
            var a = (e.originalEvent || e).clipboardData;
            if (a) {
                e.preventDefault();
                var i = a.items;
                i && this.handlePaste(i);
                var n = a.getData("text/plain");
                if (n && 0 == this.files.length) return this.insertText(n)
            }
            return !1
        }
    },
    getHTMLTemplet: function() {
        return this.isUser ? "<div id='fileintex-FILEINDEX' purpose='fileupload' CLASS><input type='text' value='' class='file-name' purp='name'/><div id='pasteimage' class='pasteimage'></div><textarea class='pasteimgcomment' id='pasteimgcomment' purp='comment' placeholder='" + LSResource.getRealValue("transferchat.comment.placeholder") + "'></textarea></div>" : $EmbedManger.UI.getPrevHTML()
    },
    getEmbPrevHTML: function() {
        return '<div id="fileintex-FILEINDEX" purpose="fileupload"><div class="imgshr_head txtelips" title="' + getEncodedText(Support.attendername) + '">' + LSResource.getRealValue("common.sharewith", getEncodedText(Support.attendername)) + '</div><input type=\'text\' value=\'\' class=\'file-name\' purp=\'name\'/><div class="embd_pstimgdiv" id="pasteimage"></div><textarea id="pasteimgcomment" purp="comment" placeholder="' + LSResource.getRealValue("common.comment.placeholder") + '"></textarea><div class="pstimg_btns"><span id="uploadimage" class="txtelips strtbtn drkdclr">' + LSResource.getRealValue("banner.share") + '</span><span id="cancelupload" class="txtelips cnsl">' + LSResource.getRealValue("common.cancel") + "</span></div></div>"
    },
    getHTML: function() {
        for (var e = "<div class='pstimg-main' id='pstimg-main'>", t = this.files.length, a = 2 < t ? "class='mult-imgs mrgn-bt18'" : 2 == t ? "class='mult-imgs'" : "", i = 0; i < t; i++) {
            var n = this.getHTMLTemplet();
            e += n = (n = n.replace(/FILEINDEX/g, i)).replace(/CLASS/g, a)
        }
        return e + "</div>"
    },
    insertText: function(e) {
        var t = document.getSelection();
        return window.clipboardData ? (document.selection.createRange().pasteHTML(e), !0) : !!(document.queryCommandEnabled("insertText") && 0 < t.rangeCount && document.execCommand("insertText", !1, e))
    },
    checkAndShowUI: function() {
        if (0 < this.files.length) {
            if (this.isUser) return void this.showPortalUI();
            $EmbedManger.UI.showPasteUI()
        }
    },
    handlePaste: function(e) {
        this.files = [], itemlen = e.length;
        for (var a = this, i = function(e) {
                var t = e.getElementsByTagName("img");
                t && 0 == t.length && (a.files = [])
            }, t = function(e) {
                var t = document.createElement("DIV");
                t.innerHTML = e, i(t)
            }, n = function(e) {
                var t = document.createElement("DIV");
                t.innerTEXT = e, i(t)
            }, s = 0; s < itemlen; s++) {
            var r = e[s].type,
                o = e[s].kind,
                l = e[s].getAsFile();
            /^image\/.+$/.test(r) && l && this.files.push(l), $Support.isMacOS() || (r && "text/plain" == r && o && "string" == o ? e[s].getAsString(n) : e[s].getAsString(t))
        }
        this.checkAndShowUI()
    },
    updateDetailes: function(e) {
        for (var t = 0, a = this.files, i = a.length; t < i; t++) {
            var n = a[t],
                s = n.name,
                r = "",
                o = e.find("#fileintex-" + t);
            s || (s = this.generateName(n, t));
            var l = s.lastIndexOf("."); - 1 !== l ? r = s.substring(l) : l = s.length, s = (s = (s = s.substring(0, l)).replace(/[`~!\$%\^&\|\\\"<>\?\/]/g, "-")).substring(0, 100), this.Filename = s, this.ext = r, o.attr("fileextn", r), o.find(".file-name").val(s), o.find("#pasteimgcomment").focus(), this.bindHandler(o), this.setPreview(o, n)
        }
    },
    bindHandler: function(e) {
        e.find(".file-name").on("blur", function() {
            "" != $(this).val() ? PasteImage.Filename = $(this).val() : $(this).val(PasteImage.Filename)
        }).on("keydown", function() {
            $(this).removeClass("bdrbot_red")
        })
    },
    generateName: function(e, t) {
        var a = e.type,
            i = a.substring(a.indexOf("/") + 1, a.length);
        return "Image - " + this.getcurrTime() + "." + i
    },
    getcurrTime: function() {
        var e = new Date,
            t = "",
            a = e.getHours(),
            i = e.getMinutes(),
            n = 12 < a ? a - 12 : 0 == a ? "12" : a;
        return t += (n < 10 ? "0" : "") + n, t += (i < 10 ? ":0" : ":") + i, t += 12 <= a ? " PM" : " AM"
    },
    showEmbedUI: function(e) {
        var t = $("#embedcontainer").find(".flcontainer"),
            a = this.getHTML();
        t.find(".pstimg-main").remove(), t.append(a).find(".pstimg-main").show(), t.find("#uploadimage").on("click", function() {
            PasteImage.uploadImage(t)
        }), t.find("#cancelupload").on("click", function() {
            PasteImage.hideUI()
        }), this.updateDetailes(t)
    },
    validate: function(e) {
        for (var t = 0, a = PasteImage.files.length; t < a; t++) {
            var i = e.find("#fileintex-" + t),
                n = i.find("[purp='name']"),
                s = i.find("[purp='comment']"),
                r = $.trim(n.val());
            if (/^[`~!\$%\^&\|\\\"<>\?\/]$/.test(r) || "" == r) return !1;
            if (100 < r.length) return !1;
            if (200 < s.val().length) return !1
        }
        return !0
    },
    uploadImage: function(e) {
        if (PasteImage.validate(e)) {
            for (var t = 0, a = PasteImage.files, i = {}, n = []; t < a.length; t++) {
                var s = a[t],
                    r = [],
                    o = e.find("#fileintex-" + t),
                    l = o.find("[purp='name']").val(),
                    m = this.comment = o.find("[purp='comment']").val(),
                    d = o.attr("fileextn");
                r.push(s), r.push(t + 1 + "_" + l + d), "" !== m && (i[t + 1] = m), n.push(r)
            }
            this.handleUploadAjax(n, i), $EmbedManger.UI.hidePasteUI()
        }
    },
    handleUploadAjax: function(a, e) {
        var t = ($Support.EmbedObj || {}).isembedrestapienabled || !1,
            i = {},
            n = this.isUser,
            s = "";
        if ("upload.do" === (n ? "upload.do" : "upload.ls") && (i = lsnocachefix("json", !0)), i.chid = n ? WindowHandler.getCurrentWinChatID() : $EmbedManger.getChatID(), i.dname = n ? CurrentUser.getDisplayname() : $EmbedManger.getVisitorName(), i.mod = "att", i.sid = n ? LSMessanger.getSid() : $EmbedManger.getLiveSupportID(), i.comment = JSON.stringify({}), 0 != getObjectSize(e) && (i.comment = JSON.stringify(e)), n) {
            var r = $("#" + i.chid).find("#chatstatus");
            r.text(LSResource.getRealValue("infomsg.sharingfiles"))
        } else i.lsid = $EmbedManger.getLiveSupportID(), i.sender = $EmbedManger.getvwmsid(), i.msgid = s = (new Date).getTime();
        i = this.getFormData(i, "file1", a);
        var o = !n && $EmbedManger.isNewEmbed;
        if (o && t) {
            var l = {
                file: a[0][0],
                client_message_id: s,
                timezone_offset: -1 * (new Date).getTimezoneOffset() * 6e4,
                session_id: LSMessanger.getSid(),
                uts_session_id: $Support.getParent().$UTSHandler.getUTSSID(),
                att_type: "chat_attachment"
            };
            i = CommonUtil.getFormData(l)
        }
        var m = {
            progress: function(e, t) {
                o && $UI.showUploadProgress({
                    fileobj: a[0][0],
                    prgval: e,
                    msgid: t,
                    filename: PasteImage.Filename + PasteImage.ext,
                    comment: PasteImage.comment
                })
            },
            complete: function() {
                o && $UI.hideUploadProgress(s)
            },
            success: function() {
                o ? PasteImage.isUser && r.text("") : $UI.hideUploadProgress(s)
            }
        };
        try {
            (o && t ? CommonUtil.v2handleFileUpload : CommonUtil.handleFileUpload)({
                callbacks: m,
                formdata: i,
                type: a[0][0].type,
                msgid: s
            })
        } catch (e) {}
    },
    isFileorBlob: function(e) {
        return e instanceof Blob
    },
    getFormData: function(e, t, a) {
        var i = new FormData;
        for (var n in e) i.append(n, e[n]);
        for (var s = 0, r = a.length; s < r; s++) {
            var o = a[s];
            this.isFileorBlob(o[0]) && i.append(t, o[0], o[1])
        }
        return i
    },
    showPortalUI: function(e) {
        var t = Visitors.getVisitorName(WindowHandler.getVistorID(WindowHandler.getCurrentWinChatID())),
            a = $(this.txtarea),
            i = Dialog.show({
                title: LSResource.getRealValue("common.sharewith", getEncodedText(t)),
                desc: this.getHTML(),
                i18n: !1,
                ok: {
                    text: LSResource.getRealValue("banner.share"),
                    callback: function() {
                        PasteImage.uploadImage(i), a.focus()
                    }
                },
                cancel: {}
            }).addClass("pasteimg_srn").center();
        this.updateDetailes(i)
    },
    hideUI: function() {
        $("#pstimg-main").fadeOut(300, function() {
            $(this).remove()
        }), $(this.txtarea).focus()
    },
    setPreview: function(e, t) {
        var a = new FileReader,
            i = e.find("#pasteimage");
        a.onload = function(e) {
            i.html('<img src="' + e.target.result + '" />')
        }, a.readAsDataURL(t)
    },
    handlepasteimage: function(e) {
        var t = parseInt(e.attr("mcount")),
            a = t - e.val().length,
            i = a < 0 ? " redtxt" : "",
            n = a < 0 ? LSResource.getRealValue("common.count.error", "" + t) : a + " ";
        e.parent().find(".messagecount").show().removeClass("redtxt").addClass(i).text(n)
    }
};